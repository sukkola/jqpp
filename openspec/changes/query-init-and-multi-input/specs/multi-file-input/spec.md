...
The system SHALL accept two or more positional file arguments and merge their parsed JSON values into a single `serde_json::Value::Array` that is used as the dataset for the session.
...
- **WHEN** one of the multiple input files contains non-JSON text
- **THEN** that file's content is represented as a JSON string value in the merged array, consistent with single-file fallback behaviour
...
- **WHEN** jqpp is launched with `users.json` and `orders.json`
- **THEN** the source label shown in the UI is `users.json, orders.json` (or a truncated form if too long)

#### Scenario: Label includes stdin for pipe + file
- **WHEN** jqpp is launched with stdin pipe and `extra.json`
- **THEN** the source label is `stdin, extra.json`
...
- **WHEN** jqpp is launched with `exists.json nonexistent.json`
- **THEN** the process prints an error identifying `nonexistent.json` and exits with status 1 before opening the TUI
