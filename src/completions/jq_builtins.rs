...
const BUILTINS: &[(&str, &str, &str, InputType)] = &[
    // ── strings ──────────────────────────────────────────────────────────────
    ("ascii_downcase",  "ascii_downcase",          "string → lowercase",          InputType::String),
    ("ascii_upcase",    "ascii_upcase",             "string → uppercase",          InputType::String),
    ("ltrimstr",        "ltrimstr(\"\")",            "remove prefix",               InputType::String),
    ("rtrimstr",        "rtrimstr(\"\")",            "remove suffix",               InputType::String),
    ("startswith",      "startswith(\"\")",          "string → bool",               InputType::String),
    ("endswith",        "endswith(\"\")",            "string → bool",               InputType::String),
    ("split",           "split(\",\")",              "split on separator",          InputType::String),
    ("test",            "test(\"\")",                "regex match → bool",          InputType::String),
    ("match",           "match(\"\")",               "regex match → object",        InputType::String),
    ("capture",         "capture(\"(?P<x>)\")",     "regex capture → object",      InputType::String),
    ("scan",            "scan(\"\")",                "all regex matches",           InputType::String),
    ("sub",             "sub(\"pat\"; \"rep\")",     "replace first match",         InputType::String),
    ("gsub",            "gsub(\"pat\"; \"rep\")",    "replace all matches",         InputType::String),
    ("explode",         "explode",                   "string → [codepoints]",       InputType::String),
    ("tonumber",        "tonumber",                  "string → number",             InputType::String),
    ("fromjson",        "fromjson",                  "JSON string → value",         InputType::String),
    ("strptime",        "strptime(\"%Y-%m-%d\")",   "parse date string",           InputType::String),
    ...
    ("isinfinite", "isinfinite",     "test for infinity",        InputType::Number),
    ("isfinite",   "isfinite",       "test for finite float",    InputType::Number),
    ("isnormal",   "isnormal",       "test for normal float",    InputType::Number),
    ("nan",        "nan",            "produce NaN",              InputType::Number),
    ("infinite",   "infinite",       "produce infinity",         InputType::Number),
    ("tostring",   "tostring",       "number → string",          InputType::Number),
    ("strftime",   "strftime(\"%Y-%m-%d\")", "format UNIX time", InputType::Number),
    ("gmtime",     "gmtime",         "UNIX ts → broken-down",    InputType::Number),
    ("ascii",      "ascii",          "codepoint → char string",  InputType::Number),

    // ── arrays ────────────────────────────────────────────────────────────────
    ("sort",       "sort",              "sort elements",                InputType::Array),
    ("sort_by",    "sort_by(.key)",     "sort by key expr",             InputType::Array),
    ("group_by",   "group_by(.key)",    "group into sub-arrays",        InputType::Array),
    ("unique",     "unique",            "deduplicate",                  InputType::Array),
    ("unique_by",  "unique_by(.key)",   "deduplicate by key",           InputType::Array),
    ("flatten",    "flatten",           "flatten nested arrays",        InputType::Array),
    ...
    ("min_by",     "min_by(.key)",      "min by key expr",              InputType::Array),
    ("max_by",     "max_by(.key)",      "max by key expr",              InputType::Array),
    ("map",        "map(.)",            "transform each element",       InputType::Array),
    ("any",        "any(. > 0)",        "test any element matches pred",InputType::Array),
    ("all",        "all(. > 0)",        "test all elements match pred", InputType::Array),
    ("first",      "first",             "first element",                InputType::Array),
    ("last",       "last",              "last element",                 InputType::Array),
    ("nth",        "nth(0)",            "nth element",                  InputType::Array),
    ("transpose",  "transpose",         "flip rows and columns",        InputType::Array),
    ("implode",    "implode",           "[codepoints] → string",        InputType::Array),
    ("from_entries","from_entries",     "[{key,value}] → object",       InputType::Array),
    ("mktime",     "mktime",            "broken-down time → UNIX",      InputType::Array),
    ("inside",     "inside([])",        "test if inside value",         InputType::Array),

    // ── objects ───────────────────────────────────────────────────────────────
    ("to_entries",    "to_entries",            "→ [{key,value}]",     InputType::Object),
    ("with_entries",  "with_entries(.value)",  "map over entries",    InputType::Object),
    ("keys_unsorted", "keys_unsorted",         "keys without sort",   InputType::Object),
    ("del",           "del(.field)",           "delete key/path",     InputType::Object),

    // ── arrays or objects ─────────────────────────────────────────────────────
    ("keys",       "keys",            "sorted keys or indices",   InputType::ArrayOrObject),
    ("values",     "values",          "values as array",          InputType::ArrayOrObject),
    ("has",        "has(\"key\")",    "test for key / index",     InputType::ArrayOrObject),
    ("map_values", "map_values(.)",   "transform each value",     InputType::ArrayOrObject),
    ("to_entries", "to_entries",      "→ [{key,value}]",          InputType::ArrayOrObject),
    ("contains",   "contains({})",    "test containment",         InputType::ArrayOrObject),
    ...
    ("length",    "length",         "count chars / elements",   InputType::NonBoolean),
    ("indices",   "indices(\"\")",  "all indices of value",     InputType::StringOrArray),
    ("index",     "index(\"\")",    "first index of value",     InputType::StringOrArray),
    ("rindex",    "rindex(\"\")",   "last index of value",      InputType::StringOrArray),
    ...
    ("leaf_paths","leaf_paths",            "paths to leaf values",    InputType::Any),
    ("getpath",   "getpath([\"key\"])",    "value at path",           InputType::Any),
    ("setpath",   "setpath([\"key\"]; .)", "set value at path",       InputType::Any),
    ("delpaths",  "delpaths([[\"k\"]])",   "delete paths",            InputType::Any),
    ("select",    "select(. != null)",     "filter — pass or empty",  InputType::Any),
    ("recurse",   "recurse",               "recursive descent",       InputType::Any),
    ("walk",      "walk(if type == \"array\" then sort else . end)", "depth-first walk", InputType::Any),
    ...
    ("first",     "first(.[])  ",                   "first output of expr",        InputType::Any),
    ("last",      "last(.[])",                       "last output of expr",         InputType::Any),
    ("range",     "range(10)",                       "0..N integer generator",      InputType::Any),
    ("reduce",    "reduce .[] as $x (0; . + $x)",   "fold / accumulate",           InputType::Any),
    ("foreach",   "foreach .[] as $x (0; . + $x)",  "stateful iteration",          InputType::Any),
    ("until",     "until(. > 10; . + 1)",            "repeat until condition",      InputType::Any),
    ("while",     "while(. < 10; . + 1)",            "repeat while condition",      InputType::Any),
    ("recurse_down", "recurse_down",                 "recursive descent (alias)",   InputType::Any),
    ...
    ("contains",  "contains(null)",                  "test containment",            InputType::Any),
    ("inside",    "inside(null)",                    "test if inside value",        InputType::Any),
    ("format",    "format(\"text\")",                "string interpolation",        InputType::Any),
...
pub fn get_completions(token: &str, input_type: Option<&str>) -> Vec<CompletionItem> {
    ...
    for is_any_pass in passes {
        for &(name, insert_text, detail, type_filter) in BUILTINS {
            ...
            if let Some(jq_type) = input_type {
                if !type_filter.compatible_with(jq_type) {
                    continue;
                }
            }
...
mod tests {
    ...
    fn string_type_shows_ascii_funcs() {
        ...
        assert!(c.iter().any(|i| i.label == "ascii_upcase"),
    ...
    fn string_type_excludes_number_only_funcs() {
        ...
        for name in &number_only {
            assert!(!c.iter().any(|i| &i.label == name),
                "{} must NOT appear for string input", name);
        }
    }
    ...
    fn number_type_shows_math_funcs() {
        ...
        for name in &["floor", "ceil", "round", "sqrt", "log"] {
            assert!(c.iter().any(|i| &i.label == name),
                "{} must appear for number input", name);
        }
    }
    ...
    fn number_type_excludes_string_only_funcs() {
        ...
        for name in &["ascii_upcase", "split", "test", "ltrimstr"] {
            assert!(!c.iter().any(|i| &i.label == name),
                "{} must NOT appear for number input", name);
        }
    }
    ...
    fn array_type_shows_sort_and_map() {
        ...
        for name in &["sort", "map", "reverse", "flatten", "unique", "first", "last"] {
            assert!(c.iter().any(|i| &i.label == name),
                "{} must appear for array input", name);
        }
    }
    ...
    fn token_prefix_filters_results() {
        ...
        assert!(c.iter().all(|i| i.label.starts_with("asc")),
            "all results must start with 'asc': {:?}", c.iter().map(|i| &i.label).collect::<Vec<_>>());
    }
    ...
    fn no_duplicates_in_output() {
        ...
        let mut seen = std::collections::HashSet::new();
        for item in &c {
            assert!(seen.insert(item.label.clone()),
    ...
    fn typed_funcs_appear_before_any_funcs_for_string() {
        ...
        if let (Some(a), Some(t)) = (pos_ascii, pos_type) {
            assert!(a < t, "type-specific funcs must precede universal ones (got ascii_upcase={}, type={})", a, t);
        }
    }
    ...
    fn insert_text_for_split_has_parens() {
        ...
        assert!(item.insert_text.contains("("),
            "split insert_text must include parens: {}", item.insert_text);
    }
    ...
    fn boolean_type_excludes_length() {
        ...
        assert!(!c.iter().any(|i| i.label == "length"),
    ...
    fn boolean_type_excludes_string_and_number_funcs() {
        let c = get_completions("", Some("boolean"));
        let excluded = ["ascii_upcase", "ascii_downcase", "split", "floor", "ceil",
                        "sqrt", "sort", "map", "to_entries", "@base64"];
        for name in &excluded {
            assert!(!c.iter().any(|i| &i.label == name),
                "{} must NOT appear for boolean input", name);
        }
    }
    ...
    fn boolean_type_shows_universal_funcs() {
        ...
        for name in &["type", "not", "tostring", "tojson", "debug", "select"] {
            assert!(c.iter().any(|i| &i.label == name),
                "{} must appear for boolean input", name);
        }
    }
    ...
    fn base64_excluded_for_non_string() {
        for jq_type in &["number", "array", "object", "boolean", "null"] {
            ...
            assert!(!c.iter().any(|i| i.label == "@base64"),
                "@base64 must NOT appear for {} input", jq_type);
        }
    }
    ...
    fn nonboolean_compatible_with_correct_types() {
        ...
        assert!(!nb.compatible_with("boolean"),
